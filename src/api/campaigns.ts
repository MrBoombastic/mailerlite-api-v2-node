  ...
  return {
    async actOnCampaign(
      ...
      return await client.post(
        `/campaigns/${campaignId}/actions/${action}`,
        data,
      );
    },
    ...
    async createCampaign(campaign: CampaignData) {
      ...
      if (campaign.type === "regular" && !campaign.subject) {
        throw new Error(
          "Mail subject is required if campaign type is regular.",
        );
      }
      ...
      if (
        campaign.type === "ab" &&
        !campaign.abSettings &&
        typeof campaign.abSettings !== "object"
      ) {
    ...
    async getCampaign(campaignId: number) {
      return await client.get(`campaigns/${campaignId}`);
    },
