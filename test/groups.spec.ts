...
describe("groups", () => {
  ...
  describe("group subscribers", () => {
    ...
    it("gets group subscriber count", async () => {
      ...
      await expect(
        client.getGroupSubscriberCount(group.id),
      ).resolves.toBeNumber();
    });

    it("adds a subscriber to a provided group", async () => {
      const { axios, client } = clientFactory();
      const axiosPost = jest.spyOn(axios, "post").mockImplementation(asyncNoop);
      ...
      expect(axiosPost).toHaveBeenCalledWith(`groups/${group.id}/subscribers`, {
        email: subscriberEmail,
        fields: {},
        name: "John Doe",
      });
    ...
    it("removes a subscriber from a provided group", async () => {
      ...
      expect(axiosDelete).toHaveBeenCalledWith(
        `groups/${group.id}/subscribers/${subscriberEmail}`,
      );
    });
  });
});
